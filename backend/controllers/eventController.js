...
export const getAllEvents = async (req, res) => {
  try {
    ...
    const [resultEvent, totalResult] = await Promise.all([
      pool.query(`SELECT * FROM events ORDER BY event_datetime LIMIT $1 OFFSET $2`, [
        perPage,
        offset,
      ]),
...
export const getEvent = async (req, res) => {
  ...
  try {
    ...
    const event = {
      ...
      time: new Date(result.rows[0].event_datetime).toTimeString().split(' ')[0],
...
export const createEvent = async (req, res) => {
  ...
  try {
    ...
    // Input validation
    if (!title || !description || !date || !time || !location || !organizer || !category) {
