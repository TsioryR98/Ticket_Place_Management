...
const SideNavbar = () => {
  ...
  return (
    ...
    >
      <button className="z-[1000]" onClick={toggleExpanded}>
        <i className={`text-black text-3xl absolute top-2 -right-10`}>
          <FaRegArrowAltCircleRight
            className={`text-black cursor-pointer text-3xl ${isExpanded && 'rotate-180'}`}
          />
        </i>
      </button>
      <Link
      ...
      >
        <h1
          className={`text-white text-2xl font-bold duration-500 transition-all ${
            isExpanded ? 'opacity-100 scale-100' : 'opacity-0 scale-0 pointer-events-none'
          }`}
        >
          Tickify
        ...
        {NavbarElements.map((element, index) => {
          return (
            <li key={index} className="mt-8">
              <Link href={element.link} className="flex items-center gap-x-8 ps-8 ">
                <element.icon
                ...
                />
                <h2
                  className={`text-orange-400 text-center font-semibold ${
                    isExpanded ? 'opacity-100 scale-100' : 'opacity-0 scale-0 pointer-events-none'
                  } duration-500 transition-all`}
                >
                  {element.name}
      ...
      {isUserLoggedIn ? (
        <div className="flex flex-col gap-y-8 mt-8 ps-8">
          <Link
            ...
            className={`flex items-center gap-x-8 text-orange-400 font-semibold text-nowrap ${
              isExpanded ? 'opacity-100 scale-100' : 'opacity-0 scale-0 pointer-events-none'
            } duration-500 transition-all`}
          >
          ...
          <button
            className={`flex items-center gap-x-8 text-orange-400 font-semibold cursor-pointer text-nowrap ${
              isExpanded ? 'opacity-100 scale-100' : 'opacity-0 scale-0 pointer-events-none'
            } duration-500 transition-all`}
          >
